...
Module RBPrjHelper
	...
	#tag Method, Flags = &h1
		Protected Function AreDataForksIdentical(f1 as FolderItem, f2 as FolderItem) As Boolean
		  ...
		  bs1 = f1.OpenAsBinaryFile (false)
		  bs2 = f2.OpenAsBinaryFile (false)
	...
	#tag Method, Flags = &h1
		Protected Sub DeleteFile(f as FolderItem, moveToTrash as Boolean = false)
		  ...
		  dim f2 as FolderItem, sh as Shell
		  if f.Exists then
		    if moveToTrash then
		      f2 = new FolderItem (f)
		      f2.MoveFileTo f2.TrashFolder
		    end
		    f.Delete
		    if f.Exists then
	...
	#tag Method, Flags = &h1
		Protected Function IsOneOf(target as String, choices() as String) As Boolean
		  return choices.IndexOf (target) >= 0
		End Function
	#tag EndMethod
