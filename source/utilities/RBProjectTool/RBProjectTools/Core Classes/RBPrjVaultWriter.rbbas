...
Inherits RBPrjTextFileWriter
	...
	#tag Method, Flags = &h21
		Private Sub updateFile(newTxt as String, srcFileRef as FolderItem, destFileRef as FolderItem, where as String)
		  ...
		  if removeFromOrigVaultFiles (destFileRef) then
		    ...
		    if srcFileRef <> nil and LenB(newTxt) = 0 then
		      ...
		      if destFileRef.Length = LenB (newTxt) then
		        tis = destFileRef.OpenAsTextFile
		        if tis <> nil then
		  ...
		  if srcFileRef <> nil and LenB(newTxt) = 0 then
		    ...
		    tos = destFileRef.CreateTextFile
		    if tos = nil then
	...
	#tag Method, Flags = &h21
		Private Sub writeContentsFile()
		  dim i as Integer
		  dim relPath, key, separator, sortKeys(), lines(), name, newTxt as String
		  dim f as FolderItem
		  ...
		  // add other file properties
		    ...
		    if inputFileRef <> nil and inputFileRef.Exists then
		      'lines.Append me.FileHdrTypeCreator + inputFileRef.MacType + "," + inputFileRef.MacCreator
		    end
		  #endif
