...
Inherits RBPrjItemsTreeBuilder
	...
	#tag Method, Flags = &h21
		Private Sub myProcessItem(containerItem as RBPrjItem)
		  ...
		  elseif RBPrjHelper.Equal (containerItem.tag, "Prop") then
		    if me.myProcessProperty (containerItem) then
		      return // no more loop over the children
		    end
		  end
		  ...
		  for each item in children
		    ...
		    elseif type = RBPrjItem.TypeString then
		      if me.textIsNonPrintable > 0 or not valueRef.IsPrintable() then
		        ...
		        // it's printable text
		        call writeItem (item, valueRef.AsText)
		      end
	...
	#tag Method, Flags = &h21
		Private Function writeItem(item as RBPrjItem, value as String = "") As XMLNode
		  dim transtag as String
		  dim result as XMLNode
		  
		  if item.type = RBPrjItem.TypeBlock then
		  ...
		  do
		    ...
		    if v = nil then
		      ...
		      if defaultResult <> "" then
		        'if RBPrjHelper.Equal (origKey, "Name") and defaultResult = "ItemName" then
		        'beep'### debug this - find another way to detect this anomaly
		        'end
	...
	#tag Method, Flags = &h21
		Private Function myProcessProperty(propItem as RBPrjItem) As Boolean
		  dim hasMoreInfo as Boolean
		  dim flags, value, declaration as String
	...
	#tag Method, Flags = &h21
		Private Sub sortProperties(items() as RBPrjItem, controlName as String)
		  ...
		  // Part of them are "PDef" group items - only these should be sorted, based on their enclosed "name" items
		  ...
		  dim order() as String
		  order = me.propOrders.Value(controlName).Split (",")
	...
	#tag Method, Flags = &h21
		Private Sub readPropOrders()
		  ...
		  // determine the line delimiters of the PropertyLists.txt file:
	...
	#tag Note, Name = About
		...
		//
		// 2. The sorting is supposed to create XML files that fully match those created by the RB IDE
		//     in their order of Property items, but the order is still wrong, and I can't figure out how
		//     to fix that (functionally, it does not matter, but it would be nice if the order matches so
		//     that it's easier to compare this output with RB's XML files for verification)
	#tag EndNote
