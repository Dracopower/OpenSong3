...
Implements RBPrjProcessing
	...
	#tag Method, Flags = &h0
		Sub ProcessHeader(dataRef as RBPrjDataRef, results as RBPrjParseResults, inputFileRef as FolderItem)
		  ...
		  if FileStart (f2, dataRef) then
		    ...
		    me.out_stream = f.CreateTextFile
		    ...
		    #if TargetMacOS
		      if inputFileRef <> nil and inputFileRef.Exists then
		        'me.out_stream.WriteLine me.FileHdrTypeCreator + inputFileRef.MacType + "," + inputFileRef.MacCreator
		      end
		    #endif
	...
	#tag Method, Flags = &h21
		Private Function makeOptimizedGroup(tag as String, ByRef linesToAppendOut() as String, ByRef needsClosure as Boolean) As String
		  ...
		  dim s, s2, optStr as String
		  dim linesToAppend() as String
		  dim lineIdx, indentState, minIndent, dummyInt as Integer
	...
	#tag Note, Name = About
		...
		//   which is used by the RBVExIm tool (http://www.rbwiki.org/doku.php/rbp/scm/rbvexim)
	...
	#tag Note, Name = File Formats
		...
		The rbvx format is now documented here:
	...
	#tag Constant, Name = FileHdrTypeCreator, Type = String, Dynamic = False, Default = \"##MacType\x2CCreator:", Scope = Public
	#tag EndConstant
