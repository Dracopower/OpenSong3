...
Protected Class SHA1
	...
	#tag Method, Flags = &h0
		 Shared Function Hash(f as FolderItem) As String
		  ...
		  return Hash(f.OpenAsBinaryFile)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		 Shared Function RunTests() As Boolean
		  if not (Hexify(Hash("")) = "DA39A3EE5E6B4B0D3255BFEF95601890AFD80709") then
		    return false
		  end if
		  
		  if not (Hexify(Hash("The quick brown fox jumps over the lazy dog")) = "2fd4e1c67a2d28fced849ee1bb76e7391b93eb12") then
		    return false
		  end if
		  
		  //The next three tests are taken from FIPS 180-1 <http://www.itl.nist.gov/fipspubs/fip180-1.htm>.
		  
		  if not (Hexify(Hash("abc")) = "A9993E364706816ABA3E25717850C26C9CD0D89D") then
		    return false
		  end if
		  
		  if not (Hexify(Hash("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq")) = "84983E441C3BD26EBAAE4AA1F95129E5E54670F1") then
		    return false
		  end if
		  
		  dim oneMillionA as String = Pad("a", 1000000)
		  if not (Hexify(Hash(oneMillionA)) = "34AA973CD4C4DAA4F61EEB2BDBAD27316534016F") then
		    return false
		  end if
		  
		  
		  //HMAC tests -- taken from http://tools.ietf.org/html/rfc2202
		  
		  if not (Hexify(HMAC(UnHexify("0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b"), "Hi There")) = "b617318655057264e28bc0b6fb378c8ef146be00") then
		    return false
		  end if
		  
		  if not (Hexify(HMAC("Jefe", "what do ya want for nothing?")) = "effcdf6ae5eb2fa2d27416d5f184df9c259a7c79") then
		    return false
		  end if
		  
		  if not (Hexify(HMAC(Unhexify("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), Pad(ChrB(&hdd), 50))) = "125d7342b9ac11cd91a39af48aa17b4f63f175d3") then
		    return false
		  end if
		  
		  if not (Hexify(HMAC(Unhexify("0102030405060708090a0b0c0d0e0f10111213141516171819"), Pad(ChrB(&hcd), 50))) = "4c9007f4026250c6bc8414f9bf50c86c2d7235da") then
		    return false
		  end if
		  
		  if not (Hexify(HMAC(Unhexify("0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c"), "Test With Truncation")) = "4c1a03424b55e07fe7f27be1d58bb9324a9a5a04") then
		    return false
		  end if
		  
		  if not (Hexify(HMAC(Pad(ChrB(&haa), 80), "Test Using Larger Than Block-Size Key - Hash Key First")) = "aa4ae5e15272d00e95705637ce8a3b55ed402112") then
		    return false
		  end if
		  
		  if not (Hexify(HMAC(Pad(ChrB(&haa), 80), "Test Using Larger Than Block-Size Key and Larger Than One Block-Size Data")) = "e8e99d0f45237d786d6bbaa7965c7808bbff1a91") then
		    return false
		  end if
	...
	#tag Note, Name = Credits
		...
		This class is taken from DeclareSub
		http://www.declaresub.com/article/130/sha-1-class
	#tag EndNote
