	...
	#tag Method, Flags = &h0
		Sub Load(f As folderItem, template As folderItem)
		  ...
		  if f.exists then
		    ...
		    'error=true
		    'errorMessage=f.NativePath+" does not exist"
	...
	#tag Method, Flags = &h0
		Sub Save(clean as boolean)
		  ...
		  if o<>nil then
		    ...
		    Else
		      errorMessage = App.T.Translate("smartml/cant_create", "Could not open preferences file for saving: %s", savePath.NativePath)
		    End If
		  end
