...
Protected Module FileUtils
	...
	#tag Method, Flags = &h1
		Protected Function AbsolutePathToFolderItem(path As String) As FolderItem
		  ...
		  Else
		    App.DebugWriter.Write("FileUtils.NativePathToFolderItem: IsPathAbsolute reported invalid path: '" + path + "'", 1)
		    f = Nil
		  End If
	...
	#tag Method, Flags = &h1
		Protected Function CopyFile(fromItem As FolderItem, toItem As FolderItem, replaceWhen As FileUtils.Replace = FileUtils.Replace.Never) As Boolean
		  Dim rs As String
		  
		  Select Case replaceWhen
		  Case Replace.Always
		    rs = kOverwriteAlways
		  Case Replace.Never
		    rs = kOverwriteNever
		  Case Replace.Newer
		    rs = kOverwriteNewer
		  End Select
		  Return CopyFile(fromItem, toItem, rs)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Function CopyFile(fromItem As FolderItem, toItem As FolderItem, overwrite As String) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function CopyPath(fromFolder As FolderItem, toFolder As FolderItem, replaceWhen As FileUtils.Replace = FileUtils.Replace.Never) As Boolean
		  Dim rs As String
		  ...
		  Case Replace.Newer
		    rs = kOverwriteNewer
		  End Select
		  Return CopyPath(fromFolder, toFolder, rs)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Function CopyPath(fromFolder As FolderItem, toFolder As FolderItem, overwrite As String) As Boolean
		  Dim i As Integer
	...
	#tag Enum, Name = Replace, Type = Integer, Flags = &h1
		Never
		  Always
		Newer
	#tag EndEnum
