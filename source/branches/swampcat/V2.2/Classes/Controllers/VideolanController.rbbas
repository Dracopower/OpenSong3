		  ...
		  If Not IsNull(mediaFile) Then
		    If mediaFile.Exists() Then
		      If InStr(parameters, "%s") = 0 Then
		        parameters =parameters + " """ + mediaFile.NativePath() + """"
		      Else
		        parameters = ReplaceAllB(parameters, "%s", """" + mediaFile.NativePath() + """")
		      End If
		      
		      If Not IsNull(videolanLocation) Then
		        If videolanLocation.Exists() Then
		          Dim cmd As String = """" + VideolanLocation.NativePath() + """  " + parameters + " vlc://quit"
