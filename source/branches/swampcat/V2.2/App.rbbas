	...
	#tag Event
		Sub Open()
		  ...
		  If AppFolder = Nil Then
		    Call MsgBox("AppFolder: """ + AppFolder.NativePath + """ is Nil", 16, "Find AppFolder")
		  End If
		  ...
		  If MyGlobals = Nil Then
		    Call MsgBox(SmartML.ErrorString + ": " + _
		    AppFolder.Child("OpenSong Settings").NativePath + _
		    " (" + Str(SmartML.ErrorCode) + ")", 16, "XDocFromFile OpenSong Settings/Globals")
		    Quit
		    Return
		  End If
		  ...
		  If Not T.IsLoaded Then
		    ...
		    If Not T.IsLoaded Then //Bail out...something's wrong
		      ...
		      InputBox.Message "Language file '" + AppFolder.Child("OpenSong Languages").Child(temp).NativePath +_
		  ...
		  Else
		    If FileUtils.CopyPath(AppFolder.Child("OpenSong Scripture"), DocsFolder.Child("Scripture"), FileUtils.Replace.Newer) Then
		      ScriptureFolder = DocsFolder.Child("Scripture")
		    End If
		  End If
		  ...
		  If Not ScriptureFolder.Exists OR ScriptureFolder.Count = 0 Then
		    App.MouseCursor = Nil
		    MsgBox T.Translate("errors/no_scripture_folder", ScriptureFolder.NativePath)
		  ...
		  If CheckDefaultFolders(DEFAULTS_FOLDER) <> FOLDER_EXISTS Then
		    App.MouseCursor = Nil
		    '++JRC Translated
		    MsgBox T.Translate("errors/no_defaults_folder", AppFolder.Child(STR_OS_DEFAULTS).NativePath)
		    '--
		    'Quit
		  End If
		  //--
		  ...
		  If result = FOLDER_EMPTY Then
		    ...
		    If InputBox.AskYN(App.T.Translate("questions/documents_folder_empty/@caption")) Then
		      If Not FileUtils.CopyPath(AppFolder.Child(STR_OS_DEFAULTS), DocsFolder) Then
		        '++JRC Translated
		        If DocsFolder <> Nil Then
		          MsgBox T.Translate("errors/no_docs_folder", DocsFolder.NativePath)
		        Else
		          MsgBox T.Translate("errors/no_docs_folder", "")
		        End If
		        '--
		  ...
		  If CheckDefaultFolders(SETTINGS_FOLDER) <> FOLDER_EXISTS Then
		    App.MouseCursor = Nil
		    '++JRC Translated
		    MsgBox  T.Translate("errors/no_settings_folder", AppFolder.Child(STR_OS_DEFAULTS).Child(STR_SETTINGS).NativePath)
		    '--
		    'Quit
		  End If
		  //--
		  ...
		  If CheckDefaultFolders(SONGS_FOLDER) <> FOLDER_EXISTS Then
		    App.MouseCursor = Nil
		    '++JRC Translated
		    MsgBox   T.Translate("errors/no_songs_folder",  AppFolder.Child(STR_OS_DEFAULTS).Child(STR_SONGS).NativePath)
		    '--
		    '++JRC Change behavior here to notify user but continue operation
		    'Quit
		  End If
		    ...
		    If  CheckDefaultFolders(SONGS_FOLDER) =  FOLDER_EXISTS Then
		      ...
		      If InputBox.AskYN(App.T.Translate("questions/songs_folder_empty/@caption")) Then
		        If Not FileUtils.CopyPath(AppFolder.Child(STR_OS_DEFAULTS).Child(STR_SONGS), DocsFolder.Child(STR_SONGS)) Then
		          '++JRC Translated
		          MsgBox T.Translate("errors/copy_default_songs",  DocsFolder.Child(STR_SONGS).NativePath)
		          '--
		          'Quit
		        End If
		      End If
		  ...
		  ElseIf result = NO_FOLDER Then
		    If DocsFolder <> Nil Then
		      MsgBox T.Translate("errors/create_songs_folder",  DocsFolder.Child(STR_SONGS).NativePath)
		    Else
		      MsgBox T.Translate("errors/no_docs_folder", "")
		    End If
		    'Quit
		  ...
		  If CheckDefaultFolders(SETS_FOLDER) <> FOLDER_EXISTS Then
		    App.MouseCursor = Nil
		    '++JRC Translated
		    MsgBox T.Translate("errors/no_sets_folder",  AppFolder.Child(STR_OS_DEFAULTS).Child(STR_SETS).NativePath)
		    '--
		    '++JRC Change behavior here to notify user but continue operation
		    'Quit
		  End If
		    ...
		    If  CheckDefaultFolders(SETS_FOLDER) =  FOLDER_EXISTS Then
		      ...
		      If InputBox.AskYN(App.T.Translate("questions/sets_folder_empty/@caption")) Then
		        If Not FileUtils.CopyPath(AppFolder.Child(STR_OS_DEFAULTS).Child(STR_SETS), DocsFolder.Child(STR_SETS)) Then
		          '++JRC Translated
		          MsgBox T.Translate("errors/create_sets_folder",  DocsFolder.Child(STR_SETS).NativePath)
		          '--
		          'Quit
		        End If
		      End If
		  ...
		  If CheckDefaultFolders(BACKGROUNDS_FOLDER) <> FOLDER_EXISTS Then
		    App.MouseCursor = Nil
		    '++JRC Translated
		    MsgBox  T.Translate("errors/no_backgrounds_folder",  AppFolder.Child(STR_OS_DEFAULTS).Child(STR_BACKGROUNDS).NativePath)
		    '--
		    '++JRC Change behavior here to notify user but continue operation
		    'Quit
		  End If
		    ...
		    If  CheckDefaultFolders(BACKGROUNDS_FOLDER) =  FOLDER_EXISTS Then
		      ...
		      If InputBox.AskYN(App.T.Translate("questions/backgrounds_folder_empty/@caption")) Then
		        If Not FileUtils.CopyPath(AppFolder.Child(STR_OS_DEFAULTS).Child(STR_BACKGROUNDS), DocsFolder.Child(STR_BACKGROUNDS)) Then
		          '++JRC Translated
		          MsgBox T.Translate("errors/create_backgrounds_folder",  DocsFolder.Child("Backgrounds").NativePath)
		          '--
		          'Quit
		        End If
		      End If
		  ...
		  If  result <> NO_FOLDER Then
		    If NOT Globals.SongActivityLog.Load(DocsFolder.Child("Settings").Child("ActivityLog.xml")) Then
		      MsgBox  T.Translate("errors/activity_disabled", DocsFolder.Child("Settings").Child("ActivityLog.xml").NativePath)  '++JRC Translated
		      Globals.SongActivityLog = Nil
		    End If
		  Else
		    If DocsFolder <> Nil Then
		      MsgBox  T.Translate("errors/activity_disabled", DocsFolder.NativePath + "Settings\ActivityLog.xml")
		    Else
		      MsgBox T.Translate("errors/no_docs_folder", "")
		    End If
		    Globals.SongActivityLog = Nil
		...
		Protected Function GetDocsFolder() As FolderItem
		  ...
		  If f = Nil Then
		    ...
		    If FolderName <> "" Then
		      ...
		      If f = Nil Then
		        // Relative path
		        For Each Folder in Split(FolderName, FileUtils.NativePathSeparator)
		          If Not FileUtils.CreateFolder(f.Child(Folder)) Then
		            e = New RuntimeException
		            e.Message = "GetDocsFolder: " + FileUtils.LastError
		            DebugWriter.Write "GetDocsFolder: Trying to create " + f.Child(Folder).NativePath + " got " + FileUtils.LastError, 1
		            Raise e
		          End If
		          f = f.Child(Folder)
		        Next
